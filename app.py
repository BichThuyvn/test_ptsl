...
# Thay đổi tỷ lệ thành [4, 5] giúp cột trái rộng rãi, không bị vỡ chữ "What-If" xuống dòng
...
# =====================================================================
# CỘT PHẢI: XỬ LÝ LOGIC VÀ HIỂN THỊ KẾT QUẢ KPI (TỐI ƯU CỠ CHỮ CHỜ)
# =====================================================================
with col_right:
    ...
    if btn_reset or (not btn_run):
        # Tạo khung chứa thông tin trạng thái chờ với font chữ nhỏ gọn, tinh tế bằng Markdown
        st.info("🔄 **Hệ thống đang ở trạng thái chờ kích hoạt tiến trình mô phỏng...**")
        st.markdown("""
        *Vui lòng thiết lập các tham số ở bảng điều khiển bên trái, sau đó nhấn nút **🚀 CHẠY MÔ PHỎNG** để hiển thị các chỉ số hiệu suất mục tiêu dưới đây.*
        """)
        
        # Tạo lưới giao diện hiển thị mẫu nhỏ gọn thay vì dùng st.metric to đùng
        r1, r2 = st.columns(2)
        r3, r4 = st.columns(2)
        r1.caption("🔹 **Tổng lượng giao dịch mô phỏng:** *Đang chờ dữ liệu...*")
        r2.caption("🔹 **Doanh thu kênh (Channel Revenue):** *Đang chờ dữ liệu...*")
        r3.caption("🔹 **Doanh thu từ phí dịch vụ:** *Đang chờ dữ liệu...*")
        r4.caption("🔹 **Điểm hiệu suất chi nhánh (Branch Score):** *Đang chờ dữ liệu...*")
    ...
    # Trường hợp 2: Người dùng nhấn nút CHẠY MÔ PHỎNG
        ...
        if fee is None:
            ...
            try:
                ...
                # Đổ dữ liệu thật tìm được lên giao diện Metric chữ to, đậm, cực nét và chuyên nghiệp
                with row1_col1: 
                ...
                # Khung thông báo kết quả thành công màu xanh lục
